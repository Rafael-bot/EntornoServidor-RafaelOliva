...
#print(cuadrados_lista_compresion([16, 1, 27.040000000000003, 9, 64]))
...
def cuadrados_bucle_explicito(l):
    ...
    return result
#print(cuadrados_bucle_explicito([4,1,5.2,3,8]))
...
def vocales_consonantes(s):
     for l in s:
         #Si el metodo find retorna un numero mayor que el 0 significa que ha encontrado la letra
          if('AEIOU'.find(l)>0):
               print(f'{l} es vocal.')
          elif('aeiou'.find(l)>0):#Si la letra es minuscula el metodo find retornara en un numero mayos que 0
               print(f'ERROR!!! ESTA LETRA {l} ES CHICA COMO TU PENE.')
          elif ('qwrtypñlkjhgfdszxcvbnm'.find(l) > 0):
              print(f'ERROR!!! ESTA LETRA {l} ES CHICA COMO TU PENE.')
          else:#Si no ha an habido ninguna conincidencia
               print(f'{l} es consonante')
#vocales_consonantes("INTELIGENCIA")
